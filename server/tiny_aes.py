...
class AES:
    ...
    def SubWord(self, _4byte_block):
        ...
        for position in range(4):
            byte = (_4byte_block >> (24 - position * 8)) & 0xFF
            i = byte >> 4
            j = byte & 0xF
            result = (result << 8) | self.S_BOX[i][j]
        return result
    ...
    def Matrix_Mul(self, M1, M2):
        ...
        for row in range(4):
            for col in range(4):
                for k in range(4):
                    M[row + col * 4] ^= self.mul(M1[row][k], M2[k + col * 4])
                M[row + col * 4] = self.mod(M[row + col * 4])
        return M
    def round_key_generator(self, key_bytes: bytes):
        if len(key_bytes) != 24:
            raise ValueError("Need 192bit")
        N = 6
        rounds = 12
        total_words = 4 * (rounds + 1)
        w = [int.from_bytes(key_bytes[i*4:(i+1)*4], byteorder='big') for i in range(N)]
        for i in range(N, total_words):
            temp = w[i - 1]
            if i % N == 0:
                temp = self.SubWord(self.RotWord(temp)) ^ self.RCon[(i // N) - 1]
            w.append(w[i - N] ^ temp)
        round_keys = []
        for r in range(rounds + 1):
            key_num = 0
            for j in range(4):
                key_num = (key_num << 32) | w[r * 4 + j]
            round_keys.append(self.num_2_16bytes(key_num))
        return round_keys
    def AddRoundKey(self, State, RoundKey):
        return self._16bytes_xor(State, RoundKey)
    def _16bytes_xor(self, b1, b2):
        return [b1[i] ^ b2[i] for i in range(16)]
    def _16bytes2num(self, b):
    ...
    def aes_decrypt(self, ciphertext, RoundKeys):
        ...
        State = self.AddRoundKey(State, RoundKeys[0])
        return State
    def aes_encrypt_cbc(self, plaintext, RoundKeys, iv):
        ciphertext = b""
        previous = iv
        for i in range(0, len(plaintext), 16):
            block = plaintext[i:i+16]
            block = bytes([block[j] ^ previous[j] for j in range(16)])
            encrypted = self.aes_encrypt(list(block), RoundKeys)
            encrypted_bytes = bytes(encrypted)
            ciphertext += encrypted_bytes
            previous = encrypted_bytes
        return ciphertext
    def aes_decrypt_cbc(self, ciphertext, RoundKeys, iv):
        plaintext = b""
        previous = iv
        for i in range(0, len(ciphertext), 16):
            block = ciphertext[i:i+16]
            decrypted = self.aes_decrypt(list(block), RoundKeys)
            decrypted_bytes = bytes(decrypted)
            plaintext_block = bytes([decrypted_bytes[j] ^ previous[j] for j in range(16)])
...
# aes = AES()
# key_str = "T8r9H1uqPRqkgMR9JHFbcJJaNxGBbaw1"
# key_bytes = base64.b64decode(key_str)
# RoundKeys = aes.round_key_generator(key_bytes)
# iv = b'\x00' * 16

# plaintext_str = ""
# plaintext_bytes = plaintext_str.encode('utf-8')
# padded = pad(plaintext_bytes)
# ciphertext = aes.aes_encrypt_cbc(padded, RoundKeys, iv)

# ciphertext_hex = ""
# ciphertext_bytes = bytes.fromhex(ciphertext_hex)
# decrypted = aes.aes_decrypt_cbc(ciphertext_bytes, RoundKeys, iv)
